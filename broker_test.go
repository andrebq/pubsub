...
func testIntPublish(t *testing.T, expectedValue int) {
	...
	waitForData := func(out chan D) {
		...
		// Typecheck required because the underlying object is an interface
...
func testFloat64Publish(t *testing.T, expectedValue float64) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	...
	waitForData := func(out chan D) {
		...
		// Typecheck required because the underlying object is an interface
		if actualValue, ok := d.(float64); !ok {
			t.Error("Invalid type")
		} else if actualValue != expectedValue {
			t.Errorf("Subscription %p got %v instead of %v", out, d, expectedValue)
		}
